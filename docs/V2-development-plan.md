...
| 迭代 | 主题 | 工期 | 完成度 |
|------|------|------|--------|
| Iteration 1 | 代码重构 + 安全加固（V1 债务清偿） | 1-2 天 | ⚠️ 60% |
| Iteration 2 | 综述生成 + 对比分析（核心新功能） | 2-3 天 | ✅ 100% |
| Iteration 3 | 知识图谱可视化 | 2-3 天 | ✅ 100% |
| Iteration 4 | 冲突发现面板 + 系统可靠性 | 1-2 天 | ⚠️ 50% |
| Iteration 5 | Citation 追溯 + 语义索引 + 文档完善 | 2-3 天 | ⚠️ 30% |
...
- [x] `api/routers/` 下每个文件 ≤300 行
- [x] LLM 调用 3 次重试后抛出明确异常
...
- [x] 输入 "Chiplet 3D 集成"，生成含 4 个维度的综述，每条事实可点击跳转 Source
- [x] 输入 2 篇 Marker 相关论文 ID，生成 ≥5 个维度的对比矩阵
- [ ] 综述信息保留率 ≥80%（对比原始论文内容）
...
| ID | 任务 | 描述 | 估计 | 状态 |
|----|------|------|------|------|
| V2-4.1 | scripts/conflict_detector.py | 同主题论文逐对对比 → LLM 判断是否存在矛盾 → 生成 Conflict 报告，自动在页面写入 [Conflict: ...] 标记 | 2h | ✅ |
| V2-4.2 | 冲突前端面板 | StatusPage.tsx 已含 `unresolved_conflicts` 检测和展示，可筛选/修复 | 2h | ✅ |
| V2-4.3 | 健康检查自动化 | HealthChecker 定时任务（配置间隔，默认 24h），结果保存为报告 | 1h | ⬜ |
| V2-4.4 | Alembic 引入 | 数据库迁移框架搭建，初始化 migration | 1h | ❌ 跳过 |
| V2-4.5 | Docker 部署 | Dockerfile + docker-compose.yml，开发/生产双模式 | 1.5h | ❌ 跳过 |

### 验收标准

- [x] 通过标签/领域匹配自动发现同主题页面
- [x] LLM 逐对分析矛盾，生成结构化 Conflict 报告
- [x] 矛盾标记自动写入对应 Wiki 页面
...
| ID | 任务 | 描述 | 估计 | 状态 |
|----|------|------|------|------|
| V2-5.1 | Citation 追溯 | citation_tracer.py: 提取 [Source:] 和 [[wikilinks]] → 构建引用图 → `build/trace/top` 三个子命令 | 2h | ✅ |
| V2-5.2 | 语义索引生成 | 自动按方向/时间/实体分组生成 index.md | 1.5h | ⬜ |
| V2-5.3 | 知识过期检测 | 对比新旧论文结论，标记过期页面 | 1.5h | ❌ 延后 |
...
V2.2.0 — I4 完成 ✅（今日：conflict_detector.py + StatusPage 冲突面板）
V2.3.0 — I5 骨架完成 ⚠️（citation_tracer.py ✅，index/doc 延后）
```
...
| 迭代 | 日期 | 完成度 | 说明 |
|------|------|--------|------|
| I1 | 2026-05-06 | 60% | 路由拆分 ✅、call_llm 重试 ✅ |
| I2 | 2026-05-07 | 100% ✅ | survey.py + compare.py + SurveyPage + ComparePage |
| I3 | 2026-05-08 | 100% ✅ | KnowledgeGraph (D3.js) + GraphPage |
| I4 | 2026-05-09 | 50% | conflict_detector.py ✅、StatusPage ✅；Docker/Alembic 跳过 |
| I5 | 2026-05-09 | 30% | citation_tracer.py ✅；语义索引 / 文档延后 |
| **V2.3** | **2026-05-10** | **85%** | 全局异常日志 ✅、FAQ 生成 50条 ✅、问答保存 ✅、知识库分页 ✅、热门查询增强 ✅、PDF 重新转换 ✅ |

---

*本文档基于 V2-requirements.md 和 v1-architecture-review.md 制定。执行方式遵循 ACP 流程。*
*最后更新：2026-05-09 — V2 开发计划状态刷新 + I4/I5 核心产物交付*